...
for i in `seq 0 24`; do
    ...
    for j in $src_main_dir/en/$i_str*/*; do
        ...
        zcat $j | sed "s/$src_base\.\([aptmw]\)\.gz/$trg_base.en.\1.gz/g" | gzip -c > $trg_dir/$trg_file;
    done;
    for j in $src_main_dir/cs/wsj$i_str*; do \
        src_file=`basename $j`; \
        src_base=`echo $src_file | sed 's/\..*$//'`;
        ...
        if [[ "$trg_file" == *.gz ]]; then
            echo $j "--->" $trg_dir/$trg_file;
            zcat $j | sed "s/$src_base\.cz\.\([aptmw]\)\(\.gz\)\?/$trg_base.cs.\1.gz/g" | gzip -c > $trg_dir/$trg_file;
        else
            echo $j "--->" $trg_dir/$trg_file.gz;
            cat $j | sed "s/$src_base\.cz\.\([aptmw]\)\(\.gz\)\?/$trg_base.cs.\1.gz/g" | gzip -c > $trg_dir/$trg_file.gz;
        fi;
    done;
done
